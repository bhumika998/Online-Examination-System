...
public class StudentsDao {
	...
	public static boolean doValidate(Students sd) {
		...
		String sql = "select * from studenttable where userid=? and password=?";         // and name=?"; //change here
		PreparedStatement pst = con.prepareStatement(sql);
		pst.setString(1, sd.getUsername());
		pst.setString(2, sd.getPassword());
				//pst.setString(3, sd.getName());   // change here
		
		ResultSet rs = pst.executeQuery();  //added sql
	...
	public static String getStudentName(String username) {
		...
		PreparedStatement pst = con.prepareStatement(sql); //sql 
		pst.setString(1,username);
	
		ResultSet rs = pst.executeQuery(); //sql added 
		if(rs.next())
		{
			name = rs.getString(1);
		}
